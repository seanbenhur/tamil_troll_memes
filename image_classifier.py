...
class DataArgs:
    ...
    train_dir_path: str = field(
        default="data/Tamil_troll_memes/train/uploaded_tamil_memes"
    )
...
training_args, data_args, model_args, wandb_args = (
    TrainingArgs,
    DataArgs,
    ModelArgs,
    WandbArgs,
...
def create_dataset(dataset_path, transforms, dir_path):
    train_ds = MemeDataset(dataset_path, transforms, dir_path)
    return train_ds
...
train_ds = create_dataset(
    data_args.train_path, data_args.train_dir_path, image_transform
)
...
class Image_Classifier(nn.Module):
    def __init__(self, model_args):
        ...
        self.model = timm.create_model(
            model_name=model_args.model_name, pretrained=model_args.pretrained
        )
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in epoch_iterator:
            ...
            logging.info(
                f"\t Val. Loss: {eval_loss:.3f} |  Val. F1: {eval_f1*100:.2f}%"
            )
        wandb.finish()
